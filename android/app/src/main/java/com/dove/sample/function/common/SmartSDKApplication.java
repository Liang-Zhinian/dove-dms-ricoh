 ...
 *  All rights reserved.
 */
package com.dove.sample.function.common;
...
public class SmartSDKApplication extends Application {
     ...
     */
    private final static String PREFIX = "common:SmartSDKApp:";
	...
	public void onCreate() {
		...
		try {
            ...
            Log.w(mTagName, PREFIX + e.toString());
			mProductId = "";
		}
	}
		...
		synchronized (contextLock) {
			...
			if(mApplicationContext == null) {
				try{
		            ...
		            Log.w(mTagName, PREFIX+ex.toString());
				}
			}
		}
	...
	public void displayAlertDialog(String appType, String state, String reason) {
		...
		Log.i(mTagName, PREFIX + "DISPLAY_ALERT_DIALOG:" + intent.getExtras().getString("STATE_REASON"));
	}
	...
	public void updateAlertDialog(String appType, String state, String reason) {
		...
		intent.putExtra("STATE_REASON", reason);
		mApplicationContext.sendBroadcast(intent);
		Log.i(mTagName, PREFIX + "UPDATE_ALERT_DIALOG:" + intent.getExtras().getString("STATE_REASON"));
	}
	...
	public void hideAlertDialog(String appType, String activityName) {
		...
		mApplicationContext.sendBroadcast(intent);
		Log.i(mTagName, PREFIX + "HIDE_ALERT_DIALOG:" + intent.getExtras().getString("ACTIVITY_NAME"));
	}
    ...
    public Boolean lockPowerMode() {
        ...
        Log.i(mTagName, PREFIX + "lockPowerMode() PACKAGE_NAME=" + packageName + ", POWER_MODE=" + PROHIBITED_POWER_MODE);

        final Bundle resultExtra = syncExecSendOrderedBroadCast(intent);
        if (resultExtra == null) {
            Log.e(mTagName, PREFIX + "LOCK_POWER_MODE request : No response.(timeout)");
            return false;
        }
    ...
    public Boolean unlockPowerMode() {
        ...
        Log.i(mTagName, PREFIX + "unlockPowerMode() PACKAGE_NAME=" + packageName + ", POWER_MODE=" + PROHIBITED_POWER_MODE);
    ...
    public Boolean unlockOffline() {
        ...
        Log.i(mTagName, PREFIX + "unlockOffline() PACKAGE_NAME=" + packageName);
    ...
    public void lockSystemReset() {
        ...
        Log.i(mTagName, PREFIX + "lockSystemReset() PACKAGE_NAME=" + packageName);
    ...
    public void unlockSystemReset() {
        ...
        Log.i(mTagName, PREFIX + "unlockSystemReset() PACKAGE_NAME=" + packageName);
    ...
    public void setAppState(String activityName, int state, String errMessage, int appType) {
        ...
        Log.i(mTagName, PREFIX + "SET_APP_STAT:" + state);
        mApplicationContext.sendBroadcast(intent, APP_CMD_PERMISSION);
    }
    ...
    public Boolean controllerStateRequest(int requestState) {
        ...
        Log.i(mTagName, PREFIX + "controllerStateRequest() PACKAGE_NAME=" + packageName + " STATE=" + requestState);

        final Bundle resultExtra = syncExecSendOrderedBroadCast(intent);
        if (resultExtra == null) {
            Log.e(mTagName, PREFIX + "CONTROLLER_STATE_REQUEST request : No response.(timeout)");
            return false;
        }
        return (Boolean) resultExtra.get("RESULT");
    }
